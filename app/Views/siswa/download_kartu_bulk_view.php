...
<head>
    ...
    <title>Download Massal Kartu Siswa</title>
...
<body>
    ...
    <div id="kartu-container">
        <?php foreach ($siswa_data as $siswa) : ?>
            <?php
            // Logika Foto
            $foto_tampil = empty($siswa->photo) ? base_url('assets/img/icon/default.png') : base_url('assets/img/siswa/' . $siswa->photo);

            // Logika QR Code
            $qr_path = FCPATH . 'assets/img/qr/siswa/' . $siswa->qr_code;
            if (empty($siswa->qr_code) || !file_exists($qr_path)) {
                $qr_tampil = "https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=" . urlencode($siswa->nisn);
            } else {
                $qr_tampil = base_url('assets/img/qr/siswa/' . $siswa->qr_code);
            }
            ?>

            <!-- Beri class .capture-area dan simpan nama file di data-filename -->
            <div class="card kartu-siswa border capture-area" data-filename="Kartu_<?= str_replace(" ", "_", $siswa->nama_siswa) ?>_<?= $siswa->nisn ?>.png">
                ...
                <table style="margin-top: 130px; position: absolute; right:130px; text-align: right; font-family: Cambria;font-size: 12px;">
                    ...
                    <tr>
                        <td><strong style="font-size: 10px;"><?= $siswa->nama_siswa ?></strong></td>
                    </tr>
                    <tr>
    ...
    <script>
        ...
        window.onload = async function() {
            ...
            zip.generateAsync({
                ...
                link.download = "Kumpulan_Kartu_Siswa.zip";
